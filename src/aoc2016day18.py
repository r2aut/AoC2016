...
def get_next_row(pattern: tuple):
    row = []
...
def char_to_bool(s: str):
    return [ch == "^" for ch in s]


def bool_to_char(bbb: list[bool]):
    return "".join(["^" if b else "." for b in bbb])


def pave_floor(pattern, row_number=None, **kwargs):
...
def calc_safe_tiles(floor, **kwargs) -> int:
...
def main():
    ...
    with Progress() as progress:
        ...
        safe_tiles1 = calc_safe_tiles(floor1, progress=progress)

        row_num = 400_000
        floor2 = pave_floor(pattern, row_num, progress=progress)
        safe_tiles2 = calc_safe_tiles(floor2, progress=progress)

    print(f"{safe_tiles1} safe tiles in {row_num} rows")
    print(f"{safe_tiles2} safe tiles in {row_num} rows")
