...
def process(program_file, compared_chips, cycle_quantity=50):
    ...
    while cycle_number:
        ...
        for line in lines:
            ...
            assert (
                parts[0] == "value" or parts[0] == "bot"
            ), "Bad value in bot's instruction"
            if parts[0] == "value":
                ...
                assert (
                    parts[10] == "bot" or parts[10] == "output"
                ), "Bad value in bot's instruction"
                if parts[10] == "bot":
    ...
    return (
        comp_results.number,
        output_bins.bins[0].bin.pop().value
        * output_bins.bins[1].bin.pop().value
        * output_bins.bins[2].bin.pop().value,
    )
